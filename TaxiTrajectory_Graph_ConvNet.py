...
A1 = pd.read_csv('/media/ysh1/Seagate Backup Plus Drive/processed_taxi/6. Spatial dependency modeling/input_nei.csv')
A1 = np.array(A1)
...
X = pd.read_csv('/media/ysh1/Seagate Backup Plus Drive/processed_taxi/5-1. mat/input.csv', header=None)
X = np.array(X)
X = np.split(X, num_graphs, axis=0)
X = np.array(X)

Y = pd.read_csv('/media/ysh1/Seagate Backup Plus Drive/processed_taxi/5-2. label/input_label.csv', header=None)
Y = np.array(Y)
Y = Y[:,1]
Y = Y.reshape(717,300)

# A, X, Y = shuffle(A, X, Y)
# train_size = round(0.9*len(A))
#
...
# date = [31,28,31,30,31,29,31,31,30,31,30,31]
...
for i in range(1,len(date)):
    ...
    if test_month == 1:
        ...
        test_X = X[sum(date[0:11]):len(X)]
        train_X = X[0:sum(date[0:11])]

        test_Y = Y[sum(date[0:11]):len(Y)]
        train_Y = Y[0:sum(date[0:11])]
    ...
    # hidden_size = 100
    # dim = 300
    batch_size = 30 # round(0.05*len(train_A))
    ...
    output = Lambda(lambda x: K.mean(x, axis=1))(
        output)  # adding a node invariant layer to make sure output does not depends upon the node order in a graph.
    output = Dense(train_Y.shape[1])(output)
    output = Activation('relu')(output)
    #
    # output = np.array(output)
    ...
    for k in range(train_size):
        print(k)
        model.fit(trainX_t[k], trainY_t[k], epochs=30, verbose=2, batch_size=30)
    ...
    temp = [test_month, np.mean(RMSE), np.mean(MAPE), (end-start)/60]
    Result = np.append(Result, temp)
    np.savetxt("/home/ysh1/PycharmProjects/thesis/results/res_"+str(i)+".csv", testPredict_rnn, delimiter=",")
